...
// Returns a CSS gradient string. Seed picks the palette; if no seed passed, uses
// Math.random so each call is fresh.
export function randomGradient(seed) {
  const idx =
    seed !== undefined
      ? hashString(String(seed)) % PALETTES.length
      : Math.floor(Math.random() * PALETTES.length);
  const palette = PALETTES[idx];
  const angle = seed !== undefined
    ? (hashString(String(seed) + "a") % 180) + 90
    : Math.floor(Math.random() * 180) + 90;
  return `linear-gradient(${angle}deg, ${palette[0]}, ${palette[1]} 50%, ${palette[2]})`;
}

// Pick a deterministic gradient for a given key. Stable across reloads.
